...
request.open(
  "GET",
  "https://www.corona-memory.ch/api/media?per_page=999999&item_set_id=3527&sort_order=desc",
  true
);
  ...
  if (request.status >= 200 && request.status < 400) {
    data.forEach((object) => {
      ...
      const media = new MediaEntry(
        obj_values[3],
        Object.values(obj_values[15])[1],
        obj_values[4],
        obj_values[9],
        obj_values[17].toString().split("/")[0],
        obj_values[23]
      );
      ...
      if (BigInt(pageId) === BigInt(media.entryId)) {
        if (media.mediaType === "video" && mediaTypeEnd[3] === "mp4") {
          ...
          setAttributes(videoPlayer, {
            src: media.mediaFileUrl.toString(),
            type: "video/mp4",
            controls: "",
            width: "100%",
            height: "100%",
    ...
    for (var i = 0; i < nextNeighbourLink.length; i++) {
      if (BigInt(nextNeighbourLink[i]) === BigInt(pageId)) {
        if (BigInt(i) === BigInt(nextNeighbourLink.length - 1)) {
          chevron_left.setAttribute(
            "href",
            "Interview_" + nextNeighbourLink[i - 1]
          );
          chevron_right.setAttribute(
            "href",
            "Interview_" + nextNeighbourLink[0]
          );
        }
        if (i === 0) {
          chevron_left.setAttribute(
            "href",
            "Interview_" + nextNeighbourLink[nextNeighbourLink.length - 1]
          );
          chevron_right.setAttribute(
            "href",
            "Interview_" + nextNeighbourLink[i + 1]
