...
request.open(
  "GET",
  "https://www.corona-memory.ch/api/items?per_page=999999&item_set_id=3527&sort_order=desc",
  true
);
    ...
    data.forEach((object) => {
        ...
        for (let i = 0; i < collections.length; i++) {
          if (collections[i] === general.isSubjof) {
            if (general.isSubjof !== "Podcastbeitrag") {
              ...
              const flexContainerById = document.getElementById(
                "id-" + general.isSubjof.replaceAll(" ", "-")
              );
      ...
      if (!containsObject(general.isSubjof, collections)) {
        ...
        if (general.isSubjof !== "Podcastbeitrag") {
          ...
          setAttributes(flexContainer, {
            class: "flex-container wrap",
            id: "id-" + general.isSubjof.replaceAll(" ", "-"),
          });

          const h4Container = document.createElement("div");
          setAttributes(h4Container, {
            class: "container",
          ...
          if (
            general.isSubjof === "«Schwarzenbach-Abstimmung»" ||
